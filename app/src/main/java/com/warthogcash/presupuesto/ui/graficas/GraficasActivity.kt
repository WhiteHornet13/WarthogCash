...
class GraficasActivity : AppCompatActivity() {
        ...
        companion object {
            const val EXTRA_ABRIR_RESUMEN_ANUAL = "extra_abrir_resumen_anual"
        }

        private lateinit var binding: ActivityGraficasBinding
        private var anios: List<Int> = emptyList()
        private var abrirResumenAnualPendiente = false

        private val viewModel: GraficasViewModel by lazy {
            val repo = (application as App).repository
            ViewModelProvider(
                this,
                FabricaViewModel { GraficasViewModel(repo) })[GraficasViewModel::class.java]
        }

        // Colores para hasta 3 series superpuestas (tipo 4): año base + 2 más.
        private val coloresSeries =
            listOf(R.color.verde_principal, R.color.azul_ocio, R.color.acento_ambar)
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            lifecycleScope.launch {
                repeatOnLifecycle(Lifecycle.State.STARTED) {
                    viewModel.aniosDisponibles.collect { lista ->
                        ...
                        if (abrirResumenAnualPendiente && anios.isNotEmpty()) {
                            abrirResumenAnualPendiente = false
                            generarGraficas()
                        }
                    }
                }
            ...
            binding.spinnerMes.adapter =
                ArrayAdapter(this, R.layout.item_spinner_text, nombresMes).apply {
                    setDropDownViewResource(R.layout.item_spinner_text)
                }

            actualizarControlesVisibles()

            abrirResumenAnualPendiente = intent.getBooleanExtra(EXTRA_ABRIR_RESUMEN_ANUAL, false)
            if (abrirResumenAnualPendiente) {
                binding.chipTipo5.isChecked = true
                actualizarControlesVisibles()
            }
        }
        ...
        private fun actualizarControlesVisibles() {
            ...
            binding.contenedorAnioUnico.visibility =
                if (tipo == 1 || tipo == 3) android.view.View.VISIBLE else android.view.View.GONE
            binding.contenedorMes.visibility =
                if (tipo == 2) android.view.View.VISIBLE else android.view.View.GONE
            binding.contenedorAniosMultiples.visibility =
                if (tipo == 2 || tipo == 4 || tipo == 5) android.view.View.VISIBLE else android.view.View.GONE
        ...
        private fun nuevaTarjeta(titulo: String): ItemGraficaCardBinding {
            val tarjeta = ItemGraficaCardBinding.inflate(
                LayoutInflater.from(this),
                binding.contenedorGraficas,
                false
            )
        ...
        private val nombresMesCortos = listOf(
            "Ene",
            "Feb",
            "Mar",
            "Abr",
        ...
        private fun agregarGraficaBarrasMeses(
            titulo: String,
            valores: FloatArray,
            colorResId: Int,
            asignado: FloatArray
            ...
            val datasetBarras =
                BarDataSet(entradasBarras, getString(R.string.graficas_gasto)).apply {
            ...
            val datasetLinea =
                LineDataSet(entradasLinea, getString(R.string.graficas_asignado)).apply {
                    color = ContextCompat.getColor(this@GraficasActivity, R.color.rojo_limite)
                    setCircleColor(
                        ContextCompat.getColor(
                            this@GraficasActivity,
                            R.color.rojo_limite
                        )
                    )
                    lineWidth = 3.5f
        ...
        private fun agregarGraficaBarrasAnios(
            titulo: String,
            valoresPorAnio: Map<Int, Float>,
            colorResId: Int
        ) {
            val tarjeta = nuevaTarjeta(titulo)
            ...
            val entradas = aniosOrdenados.mapIndexed { i, anio ->
                BarEntry(
                    i.toFloat(),
                    valoresPorAnio.getValue(anio)
                )
            }
        ...
        private fun agregarGraficaLineaMesesMultiAnio(
            titulo: String,
            aniosElegidos: List<Int>,
            extraerSerie: (Int) -> FloatArray
        ) {
            val tarjeta = nuevaTarjeta(titulo)
            val chart = LineChart(this)
            tarjeta.contenedorChart.addView(chart)
        ...
        private fun aplicarEstiloBasico(chart: com.github.mikephil.charting.charts.Chart<*>) {
            ...
            chart.axisLeftOrNull()?.textColor =
